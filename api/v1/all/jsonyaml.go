...
//
// dst must be a non-nil pointer to a struct shaped like the api package's
...
// dst.FromView.
//
// src must be a non-nil pointer to a struct shaped like the api package's
// Versioned* types.
func fromVersioned(dst fromViewer, src any) error {
	...
	// type-switch inference doesn't downgrade it when multiple versions
	// share a single view.
...
// children — including nested fork-agnostic types via their own field
// structure — are populated transparently.
...
// fields where src/dst element types differ — *all.SignedBeaconBlock /
// *deneb.SignedBeaconBlock, slices of those, etc. — copyByName recursively
// allocates and copies. Fields present on src but missing on dst (e.g.
// Version when dst is per-fork) are silently skipped.
...
// dataVersionType is the reflect.Type of version.DataVersion, used by
// propagateVersion to identify Version fields on nested versionable children.
var dataVersionType = reflect.TypeFor[version.DataVersion]()

// propagateVersion seeds the Version field on every fork-agnostic struct
// reachable from root (a pointer to a struct). It mirrors the spec/all
// package's populateVersion cascade, which is unexported and therefore not
// callable from this package: any exported struct field named Version of
// type version.DataVersion is set to v, recursing through pointers, structs,
// and slices of structs or pointers.
func propagateVersion(root any, v version.DataVersion) {
	propagateVersionValue(reflect.ValueOf(root), v)
}

// propagateVersionValue is the recursive worker for propagateVersion.
func propagateVersionValue(val reflect.Value, v version.DataVersion) {
	switch val.Kind() {
	case reflect.Pointer:
		if !val.IsNil() {
			propagateVersionValue(val.Elem(), v)
		}
	case reflect.Struct:
		t := val.Type()
		for i := range val.NumField() {
			...
			fv := val.Field(i)
			if f.Name == "Version" && f.Type == dataVersionType {
				fv.Set(reflect.ValueOf(v))

				continue
			}

			propagateVersionValue(fv, v)
		}
	case reflect.Slice:
		// Only element kinds that can contain nested structs are walked;
		// byte slices, hashes, blobs etc. are skipped.
		switch val.Type().Elem().Kind() {
		case reflect.Pointer, reflect.Struct:
			for i := range val.Len() {
				propagateVersionValue(val.Index(i), v)
			}
		default:
		}
	default:
