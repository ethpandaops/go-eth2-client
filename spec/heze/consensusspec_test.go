...
func TestConsensusSpec(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			s:    &heze.SignedExecutionPayloadBid{},
		},
		{
		...
		{
			name: "SignedInclusionList",
			s:    &heze.SignedInclusionList{},
		},
		{
	...
	baseDir := filepath.Join(os.Getenv("CONSENSUS_SPEC_TESTS_DIR"), "tests", "mainnet", "heze", "ssz_static")
